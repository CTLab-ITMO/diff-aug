...
PIPELINE_PATH = 'workflows/inpainting_SDXL_v5_release_alpha_0.76_small_fixes_api.json'
...
def modificate_workflow_v5(image_path: str, mask_path: str,
    ...
    assert Path(image_path).exists(), f'Path {image_path} does not exist'
    assert Path(mask_path).exists(), f'Path {mask_path} does not exist'
    assert Path(data_images_path).exists(), f'Path {data_images_path} does not exist'
    assert Path(data_masks_path).exists(), f'Path {data_masks_path} does not exist'
    assert Path(output_path).exists(), f'Path {output_path} does not exist'
